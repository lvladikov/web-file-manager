...
- `FM.decompressToActivePanel(itemsToExtract=null, overwrite=undefined)` — start decompression of selected archive(s) in the active panel. Optional `itemsToExtract` accepts an array of exact paths, wildcard/glob patterns (e.g. ['*.jpg','docs/*.txt']) or regular expressions (RegExp objects or '/pattern/flags' strings) to extract only matching entries; `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
- `FM.decompressToOtherPanel(itemsToExtract=null, overwrite=undefined)` — tart decompression of selected archive(s) in the other panel. Optional `itemsToExtract` accepts an array of exact paths, wildcard/glob patterns (e.g. ['*.jpg','docs/*.txt']) or regular expressions (RegExp objects or '/pattern/flags' strings) to extract only matching entries; `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
- `FM.decompressInSubfolderToActivePanel(itemsToExtract=null)` — decompress selected archive(s) into new subfolder(s) named after those archives inside the active panel. Optional `itemsToExtract` accepts an array of exact paths, wildcard/glob patterns (e.g. ['*.jpg','docs/*.txt']) or regular expressions (RegExp objects or '/pattern/flags' strings) to extract only matching entries.
- `FM.decompressInSubfolderToOtherPanel(itemsToExtract=null)` — decompress selected archive(s) into new subfolder(s) named after those archives inside the other/inactive panel. Optional `itemsToExtract` accepts an array of exact paths, wildcard/glob patterns (e.g. ['*.jpg','docs/*.txt']) or regular expressions (RegExp objects or '/pattern/flags' strings) to extract only matching entries.
- `FM.testArchive()` — runs an archive integrity test for the currently selected archive(s) in the active panel. This starts an async server job and logs progress to the console (also returns an aggregated report on completion).
- `FM.compressToActivePanel(itemsToCompress=null)` — compress the current selection into an archive placed into the active panel. Optional `itemsToCompress` is an array of basenames (wildcards or regex) used to filter which of the current selection will be compressed. If there is no selection in the active panel and `itemsToCompress` is provided, the filter will be applied across all items in that panel.
- `FM.compressToOtherPanel(itemsToCompress=null)` — compress the current selection and create the archive in the other/inactive panel. Optional `itemsToCompress` is an array of basenames (wildcards or regex) used to filter which of the current selection will be compressed. If there is no selection in the active panel and `itemsToCompress` is provided, the filter will be applied across all items in that panel.
- `FM.copyToActivePanel(itemsToCopy=null, overwrite=undefined)` — copy the currently selected items in the other (inactive) panel into the active panel. Optional `itemsToCopy` is an array of basenames to filter which of the selected items should be copied (e.g., `['file.txt']`). If there is no selection in the other panel and `itemsToCopy` is provided, the filter will be applied across all items in that panel. Optional `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
- `FM.copyToOtherPanel(itemsToCopy=null, overwrite=undefined)` — copy the currently selected items in the active panel to the other/inactive panel. Optional `itemsToCopy` is an array of basenames to filter which of the selected items should be copied (e.g., `['file.txt']`). If there is no selection in the active panel and `itemsToCopy` is provided, the filter will be applied across all items in that panel. Optional `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
- `FM.moveToActivePanel(itemsToMove=null, overwrite=undefined)` — move the currently selected items in the other (inactive) panel into the active panel. Optional `itemsToMove` is an array of basenames to filter which of the selected items should be copied (e.g., `['file.txt']`). If there is no selection in the other panel and `itemsToMove` is provided, the filter will be applied across all items in that panel. Optional `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
- `FM.moveToOtherPanel(itemsToMove=null, overwrite=undefined)` — move the currently selected items in the active panel to the other/inactive panel. Optional `itemsToMove` is an array of basenames to filter which of the selected items should be copied (e.g., `['file.txt']`). If there is no selection in the active panel and `itemsToMove` is provided, the filter will be applied across all items in that panel. Optional `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
...
- `FM.startActivePanelTerminal(startingPath?, initialCommand?)` — opens an interactive terminal for the active panel. Optionally provide `startingPath` to override the panel path (resolved relative to the panel if not absolute); this falls back to the panel path if invalid. Optionally provide `initialCommand` that will be auto-typed and executed (Enter).
- `FM.startOtherPanelTerminal(startingPath?, initialCommand?)` — opens an interactive terminal for the other (inactive) panel. Behavior mirrors `startActivePanelTerminal` but uses the other panel's path as fallback.
- `FM.multiRenameInActivePanel(items?, renameActions, previewMode=false)` — applies multiple rename operations to items in the active panel using an ordered renameActions array. Operations execute in array order. `items` (string or array) filters which items to rename (supports exact names, wildcards like `*.jpg`, or regex). If no selection and items provided, filters all panel items and renames the matches. If `previewMode` is true, shows colored console output with diff highlighting (green=added, red strikethrough=removed); if false, performs actual rename with progress modal. Supports all 8 rename action types: add_text, remove_text, find_replace, sequence, date_time, case_change, trim, and swap. See `FM.help('multiRename')` for comprehensive examples.
- `FM.multiRenameInOtherPanel(items?, renameActions, previewMode=false)` — applies multiple rename operations to items in the other (inactive) panel. Same behavior as `multiRenameInActivePanel` but operates on the other panel.
- `FM.singleRenameInActivePanel(newValue, previewMode=false)` — renames a single selected item in the active panel to the specified `newValue`. If `previewMode` is true, shows preview; if false, performs actual rename. Returns error if no item is selected or if multiple items are selected.
- `FM.singleRenameInOtherPanel(newValue, previewMode=false)` — renames a single selected item in the other (inactive) panel. Same behavior as `singleRenameInActivePanel` but operates on the other panel.
