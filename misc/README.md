...
- `FM.decompressToActivePanel(itemsToExtract=null, overwrite=undefined)` — start decompression of selected archive(s) in the active panel. Optional `itemsToExtract` is an array of paths inside the archive to extract; `overwrite` follows server overwrite policy tokens (boolean or canonical strings) or leaves interactive prompting to the UI.
- `FM.decompressToOtherPanel(itemsToExtract=null, overwrite=undefined)` — start decompression of selected archive(s) into the other/inactive panel. Parameters behave same as `decompressToActivePanel`.
- `FM.decompressInSubfolderToActivePanel(itemsToExtract=null)` — decompress selected archive(s) into new subfolder(s) named after those archives inside the active panel.
- `FM.decompressInSubfolderToOtherPanel(itemsToExtract=null)` — decompress selected archive(s) into new subfolder(s) named after those archives inside the other/inactive panel.
...
- `FM.compressToActivePanel()` — compress the current selection into an archive placed into the active panel. Starts the built-in compression flow and returns immediately (job is async).
- `FM.compressToOtherPanel()` — compress the current selection and create the archive in the other/inactive panel. Starts the built-in compression flow and returns immediately (job is async).
