...
const FilterInput = ({ filter, onFilterChange, onClose, isFiltering }) => {
  return (
    <div
    ...
    >
      <input
        ...
        autoFocus
        title="Filter items by name. Use * as a wildcard (e.g., *.jpg)."
      />
      <button
        id="filter-regex-button"
        onClick={() => onFilterChange({ ...filter, useRegex: !filter.useRegex })}
        className={`p-2 ${filter.useRegex ? 'bg-sky-600' : 'bg-gray-700'} hover:bg-sky-500`}
        title="Toggle Regular Expressions"
      >
        <Regex size={20} />
      </button>
      <button
        id="filter-case-button"
        onClick={() => onFilterChange({ ...filter, caseSensitive: !filter.caseSensitive })}
        className={`p-2 ${filter.caseSensitive ? 'bg-sky-600' : 'bg-gray-700'} hover:bg-sky-500`}
      ...
      <div className="w-8 h-8 flex items-center justify-center">
        {isFiltering ? (
          ...
          <button id="filter-close-button" onClick={onClose} className="p-2 rounded-r-md hover:bg-red-500" title="Close Filter (Esc)">
            <X size={20} />
          </button>
        )}
      </div>
